  ...
  return (
    <div className="relative max-w-2xl mx-auto">
      <div className="relative">
        <svg className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-ink-light" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <input
          ...
          className="block w-full pl-12 pr-4 py-4 border border-grace rounded-xl text-lg bg-white/95 backdrop-blur-sm placeholder-primary-dark/40 focus:outline-none focus:ring-2 focus:ring-sacred focus:border-sacred shadow-lg"
        />
      </div>
          ...
          {results.map((result, index) => (
            ...
            >
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center mr-3 ${
                result.type === 'book' ? 'bg-sacred/10 text-sacred' :
              ...
              <div>
                ...
                <div className="text-sm text-ink-muted">{result.description}</div>
              </div>
            </Link>
          ))}
        </div>
      ...
      {isFocused && query && results.length === 0 && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-xl border border-grace p-4 text-center z-50">
          <p className="text-ink-muted">No results found for &ldquo;{query}&rdquo;</p>
          <Link href="/bible-quizzes" className="text-sacred hover:underline text-sm mt-2 inline-block">
            Browse all quizzes →
          </Link>
        </div>
      )}
    ...
    <div className="bg-white border border-grace rounded-xl p-6 shadow-sm">
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        ...
        >
          ...
          <div className="text-sm text-ink-muted">Start with Genesis</div>
        </Link>
        <Link
          href="/matthew-chapters"
          className="p-4 bg-blue-50 hover:bg-sacred-light border border-sacred/20 rounded-lg transition-colors text-center"
        >
          <div className="w-10 h-10 mx-auto mb-2 bg-sacred/20 rounded-full flex items-center justify-center">
            <svg className="w-5 h-5 text-scripture" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.26 10.147a60.436 60.436 0 00-.491 6.347A48.627 48.627 0 0112 20.904a48.627 48.627 0 018.232-4.41 60.46 60.46 0 00-.491-6.347m-15.482 0a50.57 50.57 0 00-2.658-.813A59.905 59.905 0 0112 3.493a59.902 59.902 0 0110.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.697 50.697 0 0112 13.489a50.702 50.702 0 017.74-3.342" /></svg>
          </div>
          <div className="font-medium text-scripture">Meet Jesus</div>
          <div className="text-sm text-ink-muted">Start with Matthew</div>
        </Link>
        <Link
          href="/bible-quizzes"
          className="p-4 bg-scripture/5 hover:bg-scripture/10 border border-sacred/20 rounded-lg transition-colors text-center"
        >
          ...
          <div className="text-sm text-ink-muted">All books and chapters</div>
        </Link>
      </div>
    </div>
  );
