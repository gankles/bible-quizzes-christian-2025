        ...
        <div className="bg-[#FAFAF9] pb-32 min-h-screen">
            ...
            <div className="bg-white/40 border-b border-grace backdrop-blur-md">
                <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
                    <Link href="/lexicon" className="group flex items-center text-ink-muted hover:text-amber-600 transition-all text-[11px] font-bold uppercase">
                    ...
                    <div className="hidden md:flex items-center space-x-6 text-[10px] font-bold uppercase text-ink-light">
                        ...
                        <span className="text-scripture">Greek Strong's resolution index</span>
                    </div>
                </div>
            </div>
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-12">
                        <div className="max-w-3xl">
                            ...
                            <h1 className="text-5xl md:text-7xl font-display font-bold text-scripture tracking-tighter mb-6">
                                Greek <span className="text-sacred italic">Lexicon.</span>
                            </h1>
                            <p className="text-xl text-ink-muted font-medium leading-relaxed max-w-2xl">
                                Complete resolution of <span className="text-scripture font-bold">{totalCount.toLocaleString()}</span> unique Greek variables found in the New Testament manuscripts.
                            </p>
                        </div>
                        <div className="glass-panel p-10 rounded-[40px] border border-grace/50 bg-white/60 shadow-xl shrink-0 text-center md:text-left">
                            <div className="text-[10px] font-bold text-ink-light uppercase mb-2">Inventory Count</div>
                            <div className="text-5xl font-bold text-scripture">{totalCount.toLocaleString()}</div>
                            <div className="text-[9px] font-bold text-sacred uppercase mt-2">Active Data Points</div>
                        </div>
                    </div>
                </div>
            </section>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {entries.map((entry: any) => (
                            <Link
                                ...
                                className="group glass-panel p-8 rounded-[40px] border border-white bg-white hover:border-sacred/50/30 hover:shadow-2xl transition-all duration-500 flex flex-col justify-between"
                            >
                                <div>
                                    <div className="flex items-center justify-between mb-8">
                                        <span className="text-[11px] font-bold text-ink-light uppercase group-hover:text-amber-600 transition-colors">{entry.strongs}</span>
                                        <div className="w-8 h-8 rounded-xl bg-primary-light/30 flex items-center justify-center text-ink-light group-hover:bg-primary-light group-hover:text-gold-dark transition-all">
                                            <BookOpenIcon className="w-4 h-4" />
                                        </div>
                                    </div>
                                    <div className="text-4xl font-bold text-scripture mb-3 tracking-tighter group-hover:text-gold-dark transition-colors">
                                        {entry.word}
                                    </div>
                                    <div className="text-[11px] font-bold text-ink-light uppercase tracking-[0.2em] mb-4 opacity-70">
                                        {entry.transliteration}
                                    </div>
                                    <p className="text-ink-muted text-[13px] font-medium leading-relaxed mb-8 line-clamp-2">
                                        {entry.definitions?.strongs?.split(';')[0] || 'Execute full lexical breakdown...'}
                                    </p>
                                </div>
                                <div className="text-[10px] font-bold text-sacred uppercase opacity-0 group-hover:opacity-100 transition-all transform translate-x-[-10px] group-hover:translate-x-0 flex items-center">
                        ...
                        <div className="mt-24 flex items-center justify-center space-x-12">
                            {currentPage > 1 ? (
                                <Link
                                    ...
                                    className="group flex items-center text-[11px] font-bold uppercase tracking-[0.2em] text-ink-muted hover:text-amber-600 transition-all"
                                >
                            ...
                            <div className="glass-panel px-10 py-5 rounded-full border border-grace/50 bg-white/60 shadow-lg flex items-center space-x-6">
                                <span className="text-[10px] font-bold text-ink-light uppercase">Sequence Layer</span>
                                <div className="text-xl font-bold text-scripture">
                                    {currentPage} <span className="text-ink-light font-medium">/ {totalPages}</span>
                                </div>
                            </div>
            ...
            <section className="py-32 px-4">
                <div className="max-w-5xl mx-auto glass-panel p-20 rounded-[64px] border border-grace/50 bg-scripture text-center relative overflow-hidden group">
                    ...
                    <p className="text-xl text-ink-light mb-12 max-w-2xl mx-auto font-medium leading-relaxed">
