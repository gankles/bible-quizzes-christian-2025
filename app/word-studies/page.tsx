        ...
        <div className="min-h-screen bg-white">
            ...
            <nav aria-label="Breadcrumb" className="max-w-7xl mx-auto px-4 md:px-10 pt-6">
                <ol className="flex items-center gap-2 text-sm text-ink-muted">
                    ...
                    <li>
                        <span className="font-semibold text-scripture">Word Studies</span>
                    </li>
                </ol>
            </nav>

            {/* Hero Section */}
            <div className="bg-primary-light/30 py-16 md:py-24 border-b border-grace mt-4">
                <div className="max-w-7xl mx-auto px-4 md:px-10 text-center">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 text-sacred text-xs font-bold uppercase mb-6">
                        <span className="relative flex h-2 w-2">
                            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-scripture opacity-75"></span>
                            <span className="relative inline-flex rounded-full h-2 w-2 bg-scripture"></span>
                        </span>
                        Original Language Study
                    </div>
                    <h1 className="text-4xl md:text-6xl font-bold font-display text-scripture mb-6 leading-tight">
                        Word <br />
                        <span className="text-sacred">Studies</span>
                    </h1>
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-10 -mt-8">
                <div className="grid grid-cols-3 gap-4 max-w-2xl mx-auto">
                    <div className="bg-white rounded-lg border border-grace shadow-sm p-5 text-center">
                        <div className="text-3xl font-bold text-scripture">{totalWords}</div>
                        <div className="text-xs font-bold uppercase text-ink-light mt-1">Total Words</div>
                    </div>
                    <div className="bg-white rounded-lg border border-grace shadow-sm p-5 text-center">
                        <div className="text-3xl font-bold text-sacred">{greekWords.length}</div>
                        <div className="text-xs font-bold uppercase text-ink-light mt-1">Greek</div>
                    </div>
                    <div className="bg-white rounded-lg border border-grace shadow-sm p-5 text-center">
                        <div className="text-3xl font-bold text-amber-600">{hebrewWords.length}</div>
                        <div className="text-xs font-bold uppercase text-ink-light mt-1">Hebrew</div>
                    </div>
                </div>
            </div>
            ...
            <main className="max-w-7xl mx-auto px-4 md:px-10 py-16">
                ...
                {greekWords.length > 0 && (
                    <div className="mb-16">
                        <div className="flex items-center gap-3 mb-8">
                            <span className="inline-flex items-center px-3 py-1 rounded-full bg-blue-50 text-scripture text-xs font-bold uppercase">
                                Greek (Koine)
                            </span>
                            <div className="flex-1 h-px bg-grace"></div>
                            <span className="text-sm text-ink-light font-medium">{greekWords.length} words</span>
                        </div>
                ...
                {hebrewWords.length > 0 && (
                    <div className="mb-16">
                        <div className="flex items-center gap-3 mb-8">
                            ...
                            <div className="flex-1 h-px bg-grace"></div>
                            <span className="text-sm text-ink-light font-medium">{hebrewWords.length} words</span>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {totalWords === 0 && (
                    <div className="text-center py-20">
                        <h3 className="text-2xl font-bold text-scripture mb-2">No word studies found</h3>
                        <p className="text-ink-muted">Word studies are being prepared. Check back soon.</p>
                    </div>
                )}
            </main>
            ...
            <section className="bg-primary-light/30 border-t border-grace py-16">
                <div className="max-w-7xl mx-auto px-4 md:px-10">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        <Link
                            href="/lexicon"
                            className="group bg-white rounded-lg border border-grace p-6 hover:border-sacred/50 hover:shadow-lg transition-all duration-300"
                        >
                            ...
                            <h3 className="font-bold text-scripture group-hover:text-gold-dark transition-colors mb-1">
                                Biblical Lexicon
                            </h3>
...
function WordCard({ word }: { word: any }) {
    ...
    const languageBadgeBg = isGreek ? 'bg-blue-50 text-scripture' : 'bg-amber-50 text-amber-700';
    ...
    return (
        <Link
            ...
            className="group bg-white rounded-lg border border-grace p-6 hover:border-sacred/50 hover:shadow-xl transition-all duration-300 flex flex-col"
        >
            {/* Language Badge */}
            <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-xs font-mono text-ink-light">{word.strongsNumber}</span>
            </div>

            {/* Original Word */}
            <div className="mb-3">
                <div className="text-3xl font-bold text-scripture group-hover:text-gold-dark transition-colors leading-tight">
                    {word.word}
                </div>
            </div>
            ...
            <div className="mb-4">
                <span className="text-sm font-semibold text-scripture">{word.transliteration}</span>
                {word.pronunciation && (
                    <span className="text-sm text-ink-light ml-2">/{word.pronunciation}/</span>
                )}
            </div>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-grace/50">
                <div className="text-xs text-ink-light">
                    <span className="font-bold text-ink-muted">{word.usageCount}</span> occurrences in Scripture
                </div>
                <span className="text-xs font-bold text-sacred group-hover:translate-x-1 transition-transform inline-flex items-center gap-1">
                    Study
