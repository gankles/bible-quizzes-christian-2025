      ...
      <div className="min-h-screen bg-primary-light/30">
        <nav className="bg-white border-b border-grace">
          <div className="max-w-4xl mx-auto px-4 py-3">
            <ol className="flex items-center flex-wrap gap-y-1 text-sm">
              ...
              <li><Link href="/commandments" className="text-sacred hover:underline">Commandments</Link></li>
              <li className="text-ink-light mx-2">/</li>
              <li><Link href="/commandments/topic" className="text-sacred hover:underline">By Topic</Link></li>
              <li className="text-ink-light mx-2">/</li>
              <li className="text-ink-muted">{name}</li>
            </ol>
          </div>
        </nav>
        ...
        <section className="py-12 bg-gradient-to-b from-scripture to-primary-light/30">
          <div className="max-w-4xl mx-auto px-4 text-center">
            <h1 className="text-3xl md:text-4xl font-bold font-display text-scripture mb-3">
              Commandments About <span className="text-sacred">{name}</span>
            </h1>
        ...
        <div className="max-w-4xl mx-auto px-4 mt-[-1.5rem] relative z-20 mb-8">
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-white border border-grace rounded-lg shadow-sm p-4 text-center">
              <div className="text-2xl font-bold text-sacred">{commandments.length}</div>
              <div className="text-xs text-ink-muted mt-1">Total</div>
            </div>
            <div className="bg-white border border-grace rounded-lg shadow-sm p-4 text-center">
              <div className="text-2xl font-bold text-green-600">{positive.length}</div>
              <div className="text-xs text-ink-muted mt-1">Positive</div>
            </div>
            <div className="bg-white border border-grace rounded-lg shadow-sm p-4 text-center">
              <div className="text-2xl font-bold text-red-600">{negative.length}</div>
              <div className="text-xs text-ink-muted mt-1">Negative</div>
            </div>
          </div>
        </div>
        ...
        <main className="max-w-4xl mx-auto px-4 pb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            <Link href={`/topics/${topic}`} className="bg-scripture rounded-lg p-5 text-white shadow-lg flex items-center justify-between hover:bg-ink-muted transition-colors">
              <div>
                <h3 className="font-bold text-lg">Study {name} in Scripture</h3>
                <p className="text-white/80 text-xs">Verses and commentary</p>
              </div>
              <span className="bg-white text-sacred px-4 py-2 rounded-lg font-bold text-sm uppercase shrink-0 ml-3">Study</span>
            </Link>
            ...
            <div className="space-y-3">
              {commandments.map(cmd => (
                <Link
                  ...
                  className="block bg-white border border-grace rounded-xl p-5 hover:shadow-md hover:border-sacred/50 transition-all group"
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <h3 className="text-sm font-semibold text-scripture group-hover:text-gold-dark transition-colors mb-1">
                        {cmd.concept}
                      </h3>
                      <p className="text-xs text-ink-muted mb-2">{formatReference(cmd.referenceId)}</p>
          ...
          <section className="bg-grace/10 border border-grace rounded-xl p-6">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              <Link href={`/topics/${topic}`} className="text-sacred hover:underline text-sm">{name} Topic Page</Link>
              <Link href={`/characters-by-topic/${topic}`} className="text-sacred hover:underline text-sm">Characters: {name}</Link>
              {relatedSlugs.map(rs => (
                <Link key={rs} href={`/commandments/topic/${rs}`} className="text-sacred hover:underline text-sm">
                  Commandments: {formatTopicName(rs)}
                </Link>
              ))}
              <Link href="/commandments/topic" className="text-sacred hover:underline text-sm">All Commandments by Topic</Link>
