...
export default async function CommandmentPage({ params }: PageProps) {
  ...
  return (
    <>
        ...
        <div className="bg-white rounded-xl border border-grace overflow-hidden">
          <div className="p-6 border-b border-grace">
            <div className="flex items-center gap-2 mb-3">
              <BookOpenIcon className="w-5 h-5 text-ink-light" />
              <h2 className="text-lg font-bold text-scripture">Scripture</h2>
            </div>
            <blockquote className="text-scripture leading-relaxed border-l-4 border-sacred/20 pl-4 italic">
              &ldquo;{cmd.scriptureEnglish}&rdquo;
            </blockquote>
            <p className="text-sm text-ink-muted mt-2">&mdash; {ref}</p>
          </div>
          ...
          {cmd.scriptureGreek && (
            <div className="p-6 bg-blue-50/50">
              <p className="text-xs font-semibold text-sacred uppercase tracking-wider mb-2">Greek (Septuagint)</p>
          ...
          <div className="bg-white rounded-xl border border-grace p-5">
            ...
            <dl className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <dd>
                  <Link href={`/commandments/category/${categorySlug(cmd.category)}`} className="text-sacred hover:underline font-medium">
                    {cmd.category}
                  </Link>
                </dd>
              </div>
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {related.map(r => (
              <Link
                ...
                className="bg-white border border-grace rounded-lg p-3 hover:shadow-sm hover:border-sacred/50 transition-all flex items-start gap-2"
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-ink-muted">{formatReference(r.referenceId)}</p>
                </div>
              </Link>
            ))}
          </div>
        ...
        <div className="flex justify-between gap-4">
          {prev ? (
            <Link
              ...
              className="flex-1 bg-white border border-grace rounded-lg p-4 hover:shadow-sm hover:border-sacred/50 transition-all group"
            >
          ...
          {next ? (
            <Link
              ...
              className="flex-1 bg-white border border-grace rounded-lg p-4 hover:shadow-sm hover:border-sacred/50 transition-all text-right group"
            >
              <div className="flex items-center justify-end gap-2 text-sm text-ink-muted mb-1">
                <span>Next</span>
      ...
      <section className="max-w-4xl mx-auto px-4 pb-8">
        <div className="bg-blue-50 border border-sacred/20 rounded-xl p-6">
          ...
          <div className="flex flex-wrap gap-3">
            {chapterReaderSlug && (
              <Link href={`/${chapterReaderSlug}`} className="px-4 py-2 bg-scripture text-white text-sm font-medium rounded-lg hover:bg-ink-muted transition-colors flex items-center gap-2">
                <BookOpenIcon className="w-4 h-4" />
                Read {ref.split(':')[0]} with Commentary
              </Link>
            )}
            <Link href="/ten-commandments-quiz" className="px-4 py-2 bg-white text-sacred text-sm font-medium rounded-lg border border-sacred/50 hover:bg-primary-light transition-colors">
              Ten Commandments Quiz
            </Link>
            {quizSlug && (
              <Link href={`/${quizSlug}`} className="px-4 py-2 bg-white text-sacred text-sm font-medium rounded-lg border border-sacred/50 hover:bg-primary-light transition-colors">
      ...
      <section className="max-w-4xl mx-auto px-4 pb-12">
        <div className="bg-grace/10 border border-grace rounded-xl p-6">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            ...
            <Link href={`/commandments/category/${categorySlug(cmd.category)}`} className="text-sacred hover:underline text-sm">
