          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-grace">
                <th className="py-2 pr-4 text-xs font-semibold uppercase tracking-wider text-ink-muted">Place</th>
                <th className="py-2 pr-4 text-xs font-semibold uppercase tracking-wider text-ink-muted">Distance</th>
                <th className="py-2 text-xs font-semibold uppercase tracking-wider text-ink-muted">Verses</th>
              </tr>
            </thead>
            <tbody>
              {nearby.map(n => {
                ...
                return (
                  <tr key={n.slug} className="border-b border-grace/50">
                    <td className="py-3 pr-4">
                      <Link href={`/bible-places/${n.slug}`} className="font-medium text-scripture hover:text-gold-dark transition-colors">
                        {n.name}
                      </Link>
                      {nearPlace && (
                        <span className="text-xs text-ink-light ml-2 capitalize">{nearPlace.type}</span>
                      )}
                    </td>
                    <td className="py-3 pr-4 text-sm text-ink-muted">{n.distanceKm} km</td>
                    <td className="py-3 text-sm text-ink-muted">{nearPlace?.verseCount || 0}</td>
                  </tr>
                );
              })}
            </tbody>
